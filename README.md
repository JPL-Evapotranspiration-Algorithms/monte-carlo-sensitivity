...
```mermaid
flowchart TD
      A[Input Preparation and Cleaning] --> B[Baseline Calculation (Apply forward_process)]
      B --> C1{Single-Variable Perturbation?}
      C1 -- Yes --> D1[Generate n Univariate Perturbations (For each input variable)]
      C1 -- No  --> D2[Generate n Multivariate Perturbations (Joint for all input variables)]
      D1 --> E1[Replicate Input Rows and Add Perturbations]
      D2 --> E2[Replicate Input Rows and Add Joint Perturbations]
      E1 --> F[Model Evaluation (Apply forward_process to perturbed inputs)]
      E2 --> F
      F --> G[Effect Calculation (Differences & Normalization)]
      ...
      I --> J[Sensitivity Metrics Calculation (Pearson, R², Mean Change)]
